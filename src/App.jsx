    ...
    <main className="min-h-screen overflow-hidden bg-[#fff8f3] text-slate-900 [scroll-behavior:smooth]">
      ...
      <section id="testimonials" className="relative overflow-hidden bg-gradient-to-br from-[#0F766E] via-[#16C1C1] to-[#0E7490] py-10 text-white sm:py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{testimonials.map((testimonial) => <Reveal key={testimonial.name}><article className="group relative flex aspect-square h-full flex-col overflow-hidden rounded-[1.35rem] border border-white/25 bg-white/90 p-4 shadow-xl shadow-teal-950/10 backdrop-blur transition duration-500 hover:-translate-y-1 hover:border-white/50 hover:bg-white hover:shadow-2xl hover:shadow-teal-950/20"><div className="absolute -right-10 -top-10 h-24 w-24 rounded-full bg-[#16C1C1]/10 transition duration-500 group-hover:scale-125" /><div className="relative mb-4 flex gap-0.5 text-[#ff6f31]">{[...Array(5)].map((_, i) => <StarIcon key={i} className="h-3.5 w-3.5" />)}</div><p className="relative text-sm font-medium leading-6 text-slate-600">“{testimonial.text}”</p><div className="relative mt-4 border-t border-slate-100 pt-4"><h3 className="text-xs font-black text-slate-950 sm:text-sm">{testimonial.name}</h3><p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-xs sm:tracking-[0.14em]">{testimonial.role}</p></div></article></Reveal>)}</div>
        </div>
      </section>
